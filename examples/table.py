...
def make_treacl_table(nrows, ncols):
    ...
    '''
    tbl = t()
    tbl.rows = [t() for i in range(nrows)] # list of row series
    tbl.cols = [t() for i in range(ncols)] # list of column series
    for r in tbl.rows: r.s = [t() for x in tbl.cols]
    cntr = 0
    for ri,r in tbl.rows:
        for ci,c in tbl.cols:
            e = tbl.rows[ri].s[ci]
            e.v = cntr
            cntr += 1
    for ci,c in enumerate(tbl.cols): c.s = [r.s[ci] for r in tbl.rows]
    return tbl
...
if __name__ == '__main__':
    ...
    # n x m table
    tb2 = make_treacl_table(2,4)
    print_treacl_table(tb2)


# cntr = 0
# for ri,r in enumerate(tbl.rows):
#     tbl.rows[ri].s = [None,]*len(tbl.cols)
#     for ci,c in enumerate(tbl.cols):
#         newcell = t()
#         newcell.v = cntr
#         cntr += 1
#         tbl.rows[ri].s[ci] = newcell
#         # tbl.cols[ci].s[ri] = newcell
# return tbl
